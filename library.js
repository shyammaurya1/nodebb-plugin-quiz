...
plugin.init = async function (params) {
	...
	helpers.setupAdminPageRoute(app, '/admin/plugins/quiz', middleware, [], renderAdmin);
	...
	SocketAdmin.settings.savePages = async function (socket, data) {
		...
		pubsub.publish('quiz:save', data);
		await db.set('plugins:quiz', JSON.stringify(data));
	};
...
pubsub.on('quiz:save', async function (pages) {
	storeData(pages);
	await plugin.saveTemplates(pages);
});
...
async function getPages() {
	...
	const data = await db.get('plugins:quiz');
...
plugin.saveTemplates = async function (pages) {
	...
	const customTPL = await fs.promises.readFile(path.join(__dirname, 'templates/quiz.tpl'), 'utf-8');
	try {
		...
		winston.error('[plugin/quiz] Could not save templates!');
